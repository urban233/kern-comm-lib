...
def add_to_list(src_list: list, new_item: int) -> kern.Status:
  """Add two numbers."""
  if len(src_list) == 0:
    return kern_comm_lib.status.status.invalid_argument_error("List is empty!")
...
def divide(a: int, b: int) -> kern.AStatusOrElse[float]:
  """Divide two numbers."""
  if b == 0:
    return kern_comm_lib.status.status.zero_division_error("Division by zero!")
  return a / b
...
def test_status_or_contains_error() -> None:
  ...
  error_status = kern_comm_lib.status.status.invalid_argument_error("Invalid argument")
